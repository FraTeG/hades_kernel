...
#
# The boot image patcher currently only supports boot images in the standard
# Android boot image format. The ramdisk needs to be GZIP compressed. In case
# the kernel binary need to be patched, it needs to be uncompressed or
# compressed with GZIP or LZ4 (new format only).
#
...
if [ "$API" -eq "$API" ]; then
  ...
  if [ "$API" -ge "20" ]; then
    if [ "$ABILONG" = "arm64-v8a" ]; then ARCH=arm64; SYSTEMLIB=/system/lib64:/vendor/lib64; SYSTEMLIBPATH=/system/lib64; APPPROCESS64=true; fi;
    if [ "$ABILONG" = "mips64" ]; then ARCH=mips64; SYSTEMLIB=/system/lib64:/vendor/lib64; SYSTEMLIBPATH=/system/lib64; APPPROCESS64=true; fi;
    if [ "$ABILONG" = "x86_64" ]; then ARCH=x64; SYSTEMLIB=/system/lib64:/vendor/lib64; SYSTEMLIBPATH=/system/lib64; APPPROCESS64=true; fi;
    APKFOLDER=true
  ...
  if ($PATCHBOOTIMAGE); then
    ...
    if ($SYSTEM_ROOT_USED); then
      ...
      if ($CONTINUE); then
        ...
        elif (`LD_LIBRARY_PATH=$SYSTEMLIB $BIN/lz4 -d /sutmp/kernel /sutmp/kernel.d >/dev/null 2>/dev/null`); then
          KERNEL_COMPRESSED=true
          KERNEL_COMPRESSION=lz4
          rm /sutmp/kernel
          mv /sutmp/kernel.d /sutmp/kernel
        fi
      fi
      ...
      if ($CONTINUE); then
        if ($KERNEL_COMPRESSED); then
          if [ "$KERNEL_COMPRESSION" = "gzip" ]; then
            check_zero_def "- Compressing kernel (gzip)" "LD_LIBRARY_PATH=$SYSTEMLIB /su/bin/sukernel --gzip /sutmp/kernel /sutmp/kernel"
          elif [ "$KERNEL_COMPRESSION" = "lz4" ]; then
            check_zero_def "- Compressing kernel (lz4)" "LD_LIBRARY_PATH=$SYSTEMLIB $BIN/lz4 -9 /sutmp/kernel /sutmp/kernel.c"
            rm /sutmp/kernel
            mv /sutmp/kernel.c /sutmp/kernel
          fi
        fi
      fi
